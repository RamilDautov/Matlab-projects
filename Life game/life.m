...
h=guihandles(f);
guidata(f,h);
L=30; % Êîëè÷åñòâî êëåòîê ïî îäíîìó èçìåðåíèþ áóäåò (L-1)
...
if ~exist('x') || ~isnumeric(x) || ~isscalar(x) || ~isreal(x) || isnan(x) || isinf(x)...
        || x<=0
    set(h.edit,'ForegroundColor','red'); % òåêñò â ïîëå ââîäà "Çàäåðæêà" äåëàåì êðàñíûì
    err=1;
end
if err, return, end
...
while (isempty(get(h.stop_button,'UserData')))
    ...
    set(h.axes,'XTick',[]);
    set(h.axes,'YTick',[]);
    if X_N==zeros(H+1) %Ïðîâåðêà íà òî, íå îïóñòåëî ëè ïîëå
...
h=guidata(hobj);
hold off
answer=inputdlg('Ââåäèòå êîëè÷åñòâî êëåòîê âäîëü îäíîé îñè','Ñîçäàíèå íîâîãî ïîëÿ', 1, {'1'},'on');
try
    x=eval(answer{1});
end;
...
pc=pcolor(h.axes,Y);
    ...
    errordlg('Íå óäàëîñü ñîçäàòü ïîëå',...
        'Îøèáêà ñîçäàíèÿ ïîëÿ', 'modal');
end
set(h.axes,'XTick',[]);
set(h.axes,'YTick',[]);
